...
resource "random_string" "persona_address_filter_id" {
  length  = 8
  special = false
  lower   = true
  upper   = false
...
resource "google_bigquery_job" "Person_Address_Filter_Group" {
  job_id   = "rowaccesspolicy_person_address_${random_string.persona_address_filter_id.result}_group"
  location = "eu"

  query {
    query = "CREATE OR REPLACE ROW ACCESS POLICY person_address_group ON ${google_bigquery_table.Person_Address.dataset_id}.${google_bigquery_table.Person_Address.table_id} GRANT TO ('group:${var.email_group_dev}') FILTER USING (StateProvinceID = 0)"
...
resource "google_bigquery_job" "Person_Address_Filter_User" {
  job_id   = "rowaccesspolicy_person_address_${random_string.persona_address_filter_id.result}_user"
  location = "eu"

  query {
    query = "CREATE OR REPLACE ROW ACCESS POLICY person_address_user ON ${google_bigquery_table.Person_Address.dataset_id}.${google_bigquery_table.Person_Address.table_id} GRANT TO ('user:${var.email_n_nguyen}') FILTER USING (true)"
...
resource "google_bigquery_table" "Sales_Customer_Limited" {
  dataset_id          = google_bigquery_dataset.dataset.dataset_id
  table_id            = "Sales_Customer_Limited"
  deletion_protection = false
  view {
    query = "SELECT * FROM ${google_bigquery_table.Sales_Customer.dataset_id}.${google_bigquery_table.Sales_Customer.table_id} WHERE StoreID < 100"
  }
  labels = { "max_store_id" : "100" }
}
